import processing.video.*;
import java.io.File;

// Mapping de entrada:
// i = editar input
// 1..4 = elegir vertice
// flechas = mover vertice
// s = guardar

final String CONFIG_FILE = "mapping_in.json";

Capture cam;
PVector[] esquinasInput = new PVector[4];

boolean editarInput = false;
boolean dragging = false;
int esquinaSeleccionada = 0;
float handleRadius = 18;
float nudge = 0.0025;

void settings() {
  size(1280, 720, P2D);
}

void setup() {
  textureMode(IMAGE);
  setupCamera();
  cargarEsquinasInput();
}

void draw() {
  if (cam != null && cam.available()) {
    cam.read();
  }

  background(0);

  if (cam == null) {
    drawCameraError();
    return;
  }

  if (editarInput) {
    drawInputEditor();
  } else {
    drawMappedInput();
  }
}

void setupCamera() {
  String[] cameras = Capture.list();

  if (cameras == null || cameras.length == 0) {
    cam = null;
    return;
  }

  cam = new Capture(this, cameras[0]);
  cam.start();
}

void drawCameraError() {
  fill(255);
  textAlign(CENTER, CENTER);
  textSize(24);
  text("No se encontro una webcam.", width * 0.5, height * 0.5);
}

void drawMappedInput() {
  noStroke();
  beginShape();
  texture(cam);
  vertex(0, 0, esquinasInput[0].x * cam.width, esquinasInput[0].y * cam.height);
  vertex(width, 0, esquinasInput[1].x * cam.width, esquinasInput[1].y * cam.height);
  vertex(width, height, esquinasInput[2].x * cam.width, esquinasInput[2].y * cam.height);
  vertex(0, height, esquinasInput[3].x * cam.width, esquinasInput[3].y * cam.height);
  endShape(CLOSE);
}

void drawInputEditor() {
  image(cam, 0, 0, width, height);
  dibujarPoligonoIndicador(esquinasInput, color(255, 170, 0));
  drawEditorInfo("EDITANDO INPUT");
}

void dibujarPoligonoIndicador(PVector[] corners, int accent) {
  stroke(accent);
  strokeWeight(2);
  noFill();
  beginShape();

  for (int i = 0; i < corners.length; i++) {
    float px = corners[i].x * width;
    float py = corners[i].y * height;
    vertex(px, py);
  }

  endShape(CLOSE);

  for (int i = 0; i < corners.length; i++) {
    float px = corners[i].x * width;
    float py = corners[i].y * height;
    boolean active = i == esquinaSeleccionada;
    dibujarHandle(px, py, active, accent);
  }
}

void dibujarHandle(float x, float y, boolean active, int accent) {
  stroke(active ? color(255) : accent);
  strokeWeight(active ? 3 : 2);
  fill(0, 140);
  ellipse(x, y, handleRadius * 2, handleRadius * 2);
  line(x - 8, y, x + 8, y);
  line(x, y - 8, x, y + 8);
}

void drawEditorInfo(String title) {
  fill(0, 180);
  noStroke();
  rect(20, 20, 430, 80, 10);
  fill(255);
  textAlign(LEFT, TOP);
  textSize(20);
  text(title, 40, 36);
  textSize(14);
  text("i: presentar  |  1..4: vertice  |  mouse/flechas: mover  |  s: guardar", 40, 66);
}

void cargarEsquinasInput() {
  String path = sketchPath(CONFIG_FILE);
  File file = new File(path);

  if (!file.exists()) {
    setDefaultesquinasInput();
    return;
  }

  JSONObject json = loadJSONObject(path);
  JSONArray points = json.getJSONArray("input");

  for (int i = 0; i < 4; i++) {
    JSONObject point = points.getJSONObject(i);
    esquinasInput[i] = new PVector(point.getFloat("x"), point.getFloat("y"));
  }
}

void setDefaultesquinasInput() {
  esquinasInput[0] = new PVector(0.15, 0.15);
  esquinasInput[1] = new PVector(0.85, 0.15);
  esquinasInput[2] = new PVector(0.85, 0.85);
  esquinasInput[3] = new PVector(0.15, 0.85);
}

void saveesquinasInput() {
  JSONObject json = new JSONObject();
  JSONArray points = new JSONArray();

  for (int i = 0; i < 4; i++) {
    JSONObject point = new JSONObject();
    point.setFloat("x", esquinasInput[i].x);
    point.setFloat("y", esquinasInput[i].y);
    points.setJSONObject(i, point);
  }

  json.setJSONArray("input", points);
  saveJSONObject(json, sketchPath(CONFIG_FILE));
}

void keyPressed() {
  if (key == 'i' || key == 'I') {
    editarInput = !editarInput;
  }

  if (key == 's' || key == 'S') {
    saveesquinasInput();
  }

  if (key >= '1' && key <= '4') {
    esquinaSeleccionada = key - '1';
  }

  if (keyCode == LEFT) moveesquinaSeleccionada(-nudge, 0);
  if (keyCode == RIGHT) moveesquinaSeleccionada(nudge, 0);
  if (keyCode == UP) moveesquinaSeleccionada(0, -nudge);
  if (keyCode == DOWN) moveesquinaSeleccionada(0, nudge);
}

void moveesquinaSeleccionada(float dx, float dy) {
  esquinasInput[esquinaSeleccionada].x = constrain(esquinasInput[esquinaSeleccionada].x + dx, 0, 1);
  esquinasInput[esquinaSeleccionada].y = constrain(esquinasInput[esquinaSeleccionada].y + dy, 0, 1);
}

void mousePressed() {
  if (!editarInput) return;

  int corner = findCornerAt(mouseX, mouseY, esquinasInput);

  if (corner != -1) {
    esquinaSeleccionada = corner;
    dragging = true;
    updateesquinaSeleccionadaFromMouse(esquinasInput);
  }
}

void mouseDragged() {
  if (!editarInput || !dragging) return;
  updateesquinaSeleccionadaFromMouse(esquinasInput);
}

void mouseReleased() {
  dragging = false;
}

int findCornerAt(float mx, float my, PVector[] corners) {
  for (int i = 0; i < corners.length; i++) {
    float px = corners[i].x * width;
    float py = corners[i].y * height;

    if (dist(mx, my, px, py) <= handleRadius) {
      return i;
    }
  }

  return -1;
}

void updateesquinaSeleccionadaFromMouse(PVector[] corners) {
  corners[esquinaSeleccionada].x = constrain(mouseX / float(width), 0, 1);
  corners[esquinaSeleccionada].y = constrain(mouseY / float(height), 0, 1);
}
